...
public class MediasController : Controller
{
   ...
   public ActionResult GetUsersWithMediasList(bool forceRefresh = false)
   {
      try
      {
         InitSessionVariables();
   ...
   public ActionResult SetSelectedUser(int userId)
   {
      ResetMediasPaging();
   ...
   public ActionResult Delete()
   {
      ...
      if (id != 0)
      {
         ...
         if (Media != null)
         {
            if (Media.OwnerId == Models.User.ConnectedUser.Id || Models.User.ConnectedUser.IsAdmin)
            {
               // Supprimer tous les likes associés au média avant de supprimer le média
               var mediaLikes = DB.Likes.ToList().Where(l => l.MediaId == id).ToList();
               foreach (var like in mediaLikes)
               {
   ...
   public JsonResult ToggleLike(int mediaId)
   {
      ...
      Like existingLike = DB.Likes.ToList().FirstOrDefault(l => l.MediaId == mediaId && l.UserId == userId);
      ...
      {
         Like newLike = new Like { MediaId = mediaId, UserId = userId };
         if (newLike.IsValid())
         {
      ...
      Media media = DB.Medias.Get(mediaId);
      List<string> likedByUsers = media.LikedByUsers.Select(u => u.Name).ToList();
      bool isLiked = media.IsLikedByConnectedUser();
   ...
   [UserAccess(Access.View)]
   public JsonResult GetLikesInfo(int mediaId)
   {
      Media media = DB.Medias.Get(mediaId);
      if (media == null)
      {
