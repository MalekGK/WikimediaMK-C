        ...
        public override bool Delete(int userId)
        {
            try
            {
                ...
                if (userToDelete != null)
                {
                    ...
                    var userLikes = DB.Likes.ToList().Where(l => l.UserId == userId).ToList();
                    foreach (var like in userLikes)
                    {
                        DB.Likes.Delete(like.Id);
                    }

                    var userMedias = DB.Medias.ToList().Where(m => m.OwnerId == userId).ToList();
                    foreach (var media in userMedias)
                    {
                        var mediaLikes = DB.Likes.ToList().Where(l => l.MediaId == media.Id).ToList();
